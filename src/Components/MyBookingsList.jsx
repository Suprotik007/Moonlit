...
const MyBookingsList = ({ bookings, onDelete }) => {
    ...
    const canUpdateDate = (updatedDate) => {
    ...
    const handleCancel = (_id) => {
        if (!cancellationDeadline(bookings.Booked_For)) {
            Swal.fire({
                ...
                text: 'You can only cancel up to 3 days before the booking date.',
            });
            return;
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`${BACKEND_URL}/bookedRooms/${_id}`, {
                    method: 'DELETE',
                    headers: {
    ...
    const handleSetDate = (e) => {
        ...
        if (!canUpdateDate(dateOnly)) {
            Swal.fire({
                ...
                text: 'You cannot time travel to past. Please check the date.',
            });
            return;
        }

        fetch(`${BACKEND_URL}/bookedRooms/${bookings._id}`, {
            method: "PATCH",
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <div className='mb-5'>
            ...
            {isModalOpen && (
                <div className="fixed inset-0  bg-opacity-50 flex justify-center items-center z-50">
                    ...
                    >
                        ...
                        <button 
                            type='submit'
                            className="btn btn-outline border-2 mt-5 rounded-4xl w-full"
                            disabled={!formData.rating || !formData.description}
                        >
                            Post Review
                        </button>
                    </form>
            ...
            {/* Calendar Modal for Date Update */}
            {isCalenderOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <form 
                        onSubmit={handleSetDate}
                        className="bg-gray-900 p-7 rounded-xl w-11/12 max-w-md shadow-lg relative"
                    >
                        <button 
                            type="button"
                        ...
                        <h3 className="text-xl font-semibold mb-4 text-white">Update Date</h3>
                        ...
                        <button 
                            type='submit'
                            className="btn btn-outline border-2 text-white hover:text-black w-full rounded-4xl"
